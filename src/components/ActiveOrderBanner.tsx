...
import { MapPin, Bike, Store, Truck, UserCheck, X } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchActive = async () => {
      const { data } = await supabase
        .from("orders")
        .select("id, order_number, status, restaurant, driver_id")
        .eq("user_id", user.id)
  ...
  return (
    <div className="fixed inset-x-0 bottom-20 z-40 mx-auto max-w-md px-3 md:bottom-6 animate-in slide-in-from-bottom-4">
      <button
        ...
        className="group flex w-full items-center gap-3 rounded-2xl border border-primary/30 bg-card/95 px-3 py-2.5 shadow-lg backdrop-blur-xl transition-all hover:border-primary hover:shadow-xl"
      >
