      ...
      >
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`rounded-full ${status.bg} ${status.text} px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide`}
            >
              {status.label}
            </span>
          </div>
          <div className="text-right">
            ...
            <p className="text-[10px] text-[hsl(var(--driver-success))] font-bold mt-0.5">
              +R{order.delivery_fee} fee
            </p>
          </div>
        </div>
  ...
  useEffect(() => {
    ...
    all.forEach(async (order) => {
      ...
      if (geocodeCache[key] !== undefined) {
        ...
        const d = pos
          ? getDistance(driverLocation.lat, driverLocation.lng, pos.lat, pos.lng)
          : null;
      ...
      try {
        ...
        if (data?.[0]) {
          ...
          setDistances((p) => ({
            ...p,
            [order.id]: getDistance(driverLocation.lat, driverLocation.lng, pos.lat, pos.lng),
          }));
        } else {
