...
const getUrgencyColor = (minutes: number) => {
  if (minutes > 20) return { border: "border-destructive/40", bg: "bg-destructive/5", label: "Urgent", color: "text-destructive" };
  if (minutes > 10) return { border: "border-[hsl(var(--driver-warning)/0.4)]", bg: "bg-[hsl(var(--driver-warning)/0.05)]", label: "Waiting", color: "text-[hsl(var(--driver-warning))]" };
  return { border: "border-border", bg: "bg-card", label: "New", color: "text-[hsl(var(--driver-success))]" };
};
...
const getDistanceBadge = (km: number | null) => {
  ...
  if (km < 5) return { bg: "bg-[hsl(var(--driver-success)/0.1)]", text: "text-[hsl(var(--driver-success))]", label: `${km.toFixed(1)} km` };
  if (km <= 10) return { bg: "bg-[hsl(var(--driver-warning)/0.1)]", text: "text-[hsl(var(--driver-warning))]", label: `${km.toFixed(1)} km` };
  ...
  useEffect(() => {
    ...
    orders.forEach(async (order) => {
      ...
      if (geocodeCache[key] !== undefined) {
        if (geocodeCache[key]) {
          const d = getDistance(driverLocation.lat, driverLocation.lng, geocodeCache[key]!.lat, geocodeCache[key]!.lng);
          setOrderDistances(prev => ({ ...prev, [order.id]: d }));
        }
        return;
      }
      try {
        const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(order.customer_address)}&format=json&limit=1`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="rounded-2xl bg-primary/5 border border-primary/20 p-3 flex-1 mr-2">
          <p className="text-sm text-foreground font-semibold flex items-center gap-2">
            ...
            <span className="ml-auto text-xs text-muted-foreground">{filteredOrders.length} available</span>
          </p>
        </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`w-full rounded-lg px-3 py-2.5 text-left text-xs font-semibold transition-colors ${
                    sortBy === opt.key ? "bg-primary/10 text-primary" : "text-foreground hover:bg-secondary"
                  }`}
                >
                  {opt.label}
                </button>
      ...
      <div className="flex items-center gap-3 text-[10px] font-semibold px-1">
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[hsl(var(--driver-success))]" /> &lt;5 km</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[hsl(var(--driver-warning))]" /> 5–10 km</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-destructive" /> &gt;10 km</span>
      </div>
