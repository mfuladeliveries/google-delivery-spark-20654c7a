  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [{ data: prof }, { data: orders }] = await Promise.all([
        ...
        supabase
          .from("orders")
          .select("id, order_number, restaurant, total, status, created_at")
          .eq("user_id", user.id)
      ...
      <main className="mx-auto max-w-lg px-4 py-4 pb-nav md:pb-8 space-y-4">
        ...
        <div className="flex items-center gap-4 rounded-2xl border border-border bg-card p-4 shadow-card">
          ...
          <div>
            ...
            <span className="mt-1 inline-block rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-bold text-primary capitalize">
              {role}
            </span>
          </div>
        </div>
              ...
              {recentOrders.map((order) => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={`text-[10px] font-bold rounded-full px-2 py-0.5 ${statusColors[order.status] || "bg-muted text-muted-foreground"}`}
                    >
