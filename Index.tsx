...
// TEMPORARY: launch is restricted to the Mfuleni delivery area only.
// Restaurants outside this area stay hidden even if they're active and the
// customer's resolved zone (`currentZone`) is a different one — remove this
// restriction once other areas are ready to go live, and let `currentZone`
// drive area matching on its own.
const LAUNCH_AREA_ID = "710be50f-0238-4ea2-b492-d565b307a93a";
...
const Index = () => {
  ...
  // The "address updated" banner is only meant to show for 3.5s. Reading
  // Date.now() at render time isn't enough to hide it on schedule — nothing
  // was forcing a re-render once the window elapsed, so the banner would
  // stick around indefinitely until some unrelated state change (typing in
  // search, toggling a filter) happened to re-render this component. This
  // effect owns the clock instead.
  useEffect(() => {
    if (manualUpdatedAt == null) return;
    const timer = setTimeout(() => setManualUpdatedAt(null), 3500);
  ...
  const filtered = annotated.filter((r) => {
    ...
    const matchesArea = r.area_id === LAUNCH_AREA_ID;
