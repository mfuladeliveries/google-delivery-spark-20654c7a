...
ALTER TABLE public.restaurant_locations
  DROP CONSTRAINT IF EXISTS restaurant_locations_fee_nonneg;
ALTER TABLE public.restaurant_locations
  ADD CONSTRAINT restaurant_locations_fee_nonneg CHECK (delivery_fee IS NULL OR delivery_fee >= 0);

-- One branch per restaurant per delivery area.
DELETE FROM public.restaurant_locations a
USING public.restaurant_locations b
WHERE a.restaurant_id = b.restaurant_id
  AND a.area_id = b.area_id
  AND a.area_id IS NOT NULL
  AND a.ctid > b.ctid;

CREATE UNIQUE INDEX IF NOT EXISTS restaurant_locations_restaurant_area_uniq
  ON public.restaurant_locations (restaurant_id, area_id)
  WHERE area_id IS NOT NULL;

CREATE OR REPLACE FUNCTION public.create_verified_order(p_items jsonb, p_restaurant_name text, p_customer_name text, p_customer_contact text, p_customer_address text, p_customer_lat double precision, p_customer_lng double precision, p_special_notes text DEFAULT ''::text, p_tip numeric DEFAULT 0, p_delivery_code text DEFAULT '0000'::text, p_payment_method text DEFAULT 'online'::text, p_restaurant_id uuid DEFAULT NULL::uuid, p_restaurant_location_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_loc.id IS NOT NULL THEN
    ...
    IF NOT COALESCE(v_loc.is_open, true) THEN
      RAISE EXCEPTION 'This branch is closed right now. Please try again later.' USING ERRCODE = '22023';
    END IF;
  ...
  -- A branch may override the zone fee with its own flat delivery fee.
  IF v_loc.id IS NOT NULL AND v_loc.delivery_fee IS NOT NULL THEN
    v_delivery_fee := v_loc.delivery_fee;
  END IF;
