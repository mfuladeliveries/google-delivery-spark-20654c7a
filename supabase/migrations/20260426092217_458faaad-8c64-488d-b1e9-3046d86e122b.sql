-- Tighten dispatch timing: 3-minute offer windows, 6-minute broadcast flip
...
DECLARE
  ...
  v_new_phase text;
  v_offer_seconds integer := 180; -- 3 minutes per offer
  v_max_km numeric := 10;
...
-- Flip to broadcast after 6 minutes (2 x 3min offers) instead of 10
...
BEGIN
  ...
  WITH flipped AS (
    ...
    WHERE dispatch_phase = 'waiting'
      AND dispatch_started_at < now() - interval '6 minutes'
      AND status = 'ready'
      AND driver_id IS NULL
    RETURNING id
  )
